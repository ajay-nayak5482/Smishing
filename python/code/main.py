...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.exists(INITIAL_MODEL_PATH):
            print(f"\n--- Loading initial model from checkpoint: {INITIAL_MODEL_PATH} ---")
            initial_trained_model = tf.keras.models.load_model(
                INITIAL_MODEL_PATH, 
                custom_objects={'TFDistilBertModel': TFDistilBertModel, 'DistilBertKerasModel': DistilBertKerasModel} # Add custom class
            )
            # Re-evaluate to show its performance
            initial_test_inputs = {
